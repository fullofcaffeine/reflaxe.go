...
- keep a focused set of high-signal semantic fixtures:
  ...
  - deterministic `StringBuf`/`DateTools`/`Lambda` subset parity (`add`/`addChar`/`addSub`, `DateTools.delta` with duration helpers and `%Y-%m-%d %H:%M:%S` formatting, iterable `Lambda.map`)
