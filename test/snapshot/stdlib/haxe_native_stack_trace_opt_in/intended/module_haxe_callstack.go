...
func haxe___CallStack__CallStack_Impl__itemToString(item *haxe__StackItem) *string {
	...
	switch item.tag {
	case 0:
		hx_switch_2 = hxrt.StringFromLiteral("a C function")
	case 1:
		_g := item.params[0].(*string)
		m := _g
		hx_switch_2 = hxrt.StringConcatStringPtr(hxrt.StringFromLiteral("module "), m)
	case 2:
		...
		if inner == nil {
			...
			hx_if_3 = hxrt.StringConcatStringPtr(hxrt.StringConcatStringPtr(haxe___CallStack__CallStack_Impl__itemToString(inner), hxrt.StringFromLiteral(" (")), file)
		}
		rendered := hx_if_3
		...
		hx_switch_2 = rendered
	case 3:
		_g_2 := item.params[0].(*string)
		_g1_1 := item.params[1].(*string)
		classname := _g_2
		method := _g1_1
		hx_switch_2 = hxrt.StringConcatStringPtr(hxrt.StringConcatStringPtr(func() *string {
			var hx_if_4 *string
			if hxrt.StringEqualStringPtr(classname, nil) {
				hx_if_4 = hxrt.StringFromLiteral("<unknown>")
			} else {
				hx_if_4 = classname
			}
			return hx_if_4
	...
	case 4:
		...
		var v any = _g_3
		hx_switch_2 = hxrt.StringConcatStringPtr(hxrt.StringFromLiteral("local function #"), hxrt.StdString(v))
	}
	return hx_switch_2
}
...
func haxe___CallStack__CallStack_Impl__toString(stack any) *string {
	...
	_g := 0
	_g1 := func(hx_value_5 any) []*haxe__StackItem {
		if hx_value_5 == nil {
			var hx_zero_6 []*haxe__StackItem
			return hx_zero_6
		}
		return hx_value_5.([]*haxe__StackItem)
	}(stack)
	for _g < len(_g1) {
		item := _g1[_g]
		_g = int(int32((_g + 1)))
