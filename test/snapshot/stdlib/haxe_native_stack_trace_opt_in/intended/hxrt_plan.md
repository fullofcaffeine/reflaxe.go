...
- `exception` -> `exception.go`
  - `baseline` (`compiler_baseline`)
- `stack` -> `stack.go`
  - `class_usage` (`hxrt.stack.NativeStack`)
  - `class_usage` (`hxrt.stack.NativeStackFrame`)
  - `define` (`reflaxe_go_native_stack_trace`)
...
- `Main` | location `Main:3` | usage `variable_type` | `haxe.Array` v1 | used type `{"kind":"class","path":"Array","parameters":[],"arguments":[],"returnType":null,"fields":[]}` | eligibility `rejected:shape_mismatch` | eligibility detail `Observed type shape does not match the contract pattern.` | selection `fallback:registry_rejected` | representation `hxrt_array` | fallback `shape_mismatch: Observed type shape does not match the contract pattern.` | imports `none` | runtime `array` | no-hxrt contract `conditional` | selected no-hxrt eligible `no`
