...
func main() {
	...
	var v any = any(doc.toString())
	hxrt.Println(v)
	var v_1 any = any(parsed.firstElement().firstElement().get(hxrt.StringFromLiteral("a")))
	hxrt.Println(v_1)
	var v_2 any = any(parsed.toString())
	hxrt.Println(v_2)
	var v_3 any = any(hxrt.StringConcatStringPtr(hxrt.StringFromLiteral("type="), _Xml__XmlType_Impl__toString(hxrt.IntFromNullableAny(parsedCData.firstElement().firstChild().nodeType))))
	hxrt.Println(v_3)
	var v_4 any = any(parsedCData.toString())
	hxrt.Println(v_4)
}
