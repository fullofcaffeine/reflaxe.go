...
func (self *haxe__io__Bytes) compare(other *haxe__io__Bytes) int {
	...
	if self.length < other.length {
		hx_if_25 = self.length
	} else {
		hx_if_25 = other.length
	}
	limit := hx_if_25
	_g := 0
	_g1 := limit
...
func (self *haxe__io__Bytes) getString(pos int, len int, encoding *haxe__io__Encoding) *string {
	...
	return hxrt.StdString(hxrt.BytesStringFromView(self.__hx_this.__hx_nativeView(), pos, len, (encoding == haxe__io__Encoding_RawNative)))
}
...
func haxe__io__Bytes_ofString(value *string, encoding *haxe__io__Encoding) *haxe__io__Bytes {
	view := hxrt.BytesViewFromString(value, (encoding == haxe__io__Encoding_RawNative))
