import sys.net.Host;
import sys.ssl.Certificate;
import sys.ssl.Key;
import sys.ssl.Socket;
import sys.thread.Deque;
import sys.thread.Thread;

class Main {
	static final DEFAULT_CERT_PEM = "-----BEGIN CERTIFICATE-----\\nVCpI190WWaS+Cz+9vGVNBSsRYjgN\n-----END CERTIFICATE-----\n";
	static final DEFAULT_KEY_PEM = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";
	static final SNI_CERT_PEM = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n";
	static final SNI_KEY_PEM = "-----BEGIN PRIVATE KEY-----\\nKOG6OBx2h7HDGnJcWJNLuBbJ\n-----END PRIVATE KEY-----\n";

	static function main() {
		var defaultCert = Certificate.fromString(DEFAULT_CERT_PEM);
		var defaultKey = Key.readPEM(DEFAULT_KEY_PEM, false);
		var sniCert = Certificate.fromString(SNI_CERT_PEM);
		var sniKey = Key.readPEM(SNI_KEY_PEM, false);

		var server:Socket = new Socket();
		server.setCertificate(defaultCert, defaultKey);
		server.addSNICertificate(function(name) return name == "sni.local", sniCert, sniKey);
		server.bind(new Host("127.0.0.1"), 0);
		server.listen(2);

		var bound = server.host();
		connectAndPrint(server, bound.port, "default.local");
		connectAndPrint(server, bound.port, "sni.local");
		server.close();
	}

	static function connectAndPrint(server:Socket, port:Int, hostname:String):Void {
		var acceptedSockets = new Deque<sys.net.Socket>();
		Thread.create(function() acceptedSockets.add(server.accept()));
		var client:Socket = new Socket();
		client.verifyCert = false;
		client.setHostname(hostname);
		client.connect(new Host("127.0.0.1"), port);
		client.handshake();
		var peer = client.peerCertificate();
		Sys.println(hostname + "=" + peer.commonName);
		acceptedSockets.pop(true).close();
		client.close();
	}
}
