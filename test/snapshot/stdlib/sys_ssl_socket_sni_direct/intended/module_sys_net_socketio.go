...
func (self *sys__net__SocketOutput) writeBytes(bytes *haxe__io__Bytes, pos int, length int) int {
	...
	result := hxrt.SocketWriteValues(self.handle, func(hx_lambda_raw_41 []any) []int {
		hx_lambda_out_42 := make([]int, 0, len(hx_lambda_raw_41))
		for _, hx_lambda_item_43 := range hx_lambda_raw_41 {
			hx_lambda_out_42 = append(hx_lambda_out_42, func(hx_value_44 any) int {
				if hx_value_44 == nil {
					var hx_zero_45 int
					return hx_zero_45
				}
