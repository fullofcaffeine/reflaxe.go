...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_48 := false
		var hx_try_value_49 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
			...
			switch hx_typed_51 := hx_caught_50.(type) {
			case *haxe__io__Eof:
				e := hx_typed_51
				...
				hx_try_value_49 = partial
				hx_try_return_48 = true
				return
			default:
				hxrt.Throw(hx_caught_50)
			}
		})
		if hx_try_return_48 {
			return hx_try_value_49
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteInput(self haxe__io__Output, i haxe__io__Input, bufsize int) {
	...
	for true {
		...
		hxrt.TryCatch(func() {
			...
			switch hx_typed_53 := hx_caught_52.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_53
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_52)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_54 *haxe__io__Bytes
	if encoding == nil {
		hx_if_54 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_54 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_54
