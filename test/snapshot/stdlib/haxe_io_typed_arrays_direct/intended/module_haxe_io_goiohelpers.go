...
func haxe__io__GoIoHelpers_inputReadAll(self haxe__io__Input, bufsize int) *haxe__io__Bytes {
	...
	for true {
		...
		hxrt.TryCatch(func() {
			...
			switch hx_typed_127 := hx_caught_126.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_127
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_126)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_128 := false
		var hx_try_value_129 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
		...
		}, func(hx_caught_130 any) {
			switch hx_typed_131 := hx_caught_130.(type) {
			case *haxe__io__Eof:
				e := hx_typed_131
				...
				hx_try_value_129 = partial
				hx_try_return_128 = true
				return
			default:
				hxrt.Throw(hx_caught_130)
			}
		})
		if hx_try_return_128 {
			return hx_try_value_129
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_134 *haxe__io__Bytes
	if encoding == nil {
		hx_if_134 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_134 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_134
