...
func main() {
	...
	cwd := Sys_getCwd()
	setCwdFn(cwd)
	printlnFn(hxrt.StringConcatStringPtr(hxrt.StringFromLiteral("cwd="), hxrt.StdString(hxrt.StringEqualStringPtr(Sys_getCwd(), cwd))))
	started := timeFn()
	Sys_sleep(0.01)
	finished := timeFn()
	...
	stdout := stdoutFn()
	stdout.writeString(hxrt.StringFromLiteral("stdout=ok\n"))
	stdout.flush()
	stdout.close()
	stdoutFn().writeString(hxrt.StringFromLiteral("stdoutAfterClose=ok\n"))
}
