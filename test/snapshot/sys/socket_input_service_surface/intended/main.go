...
func consume(socket *sys__net__Socket) *string {
	...
	var parsed any = hxrt.StdParseInt(hxrt.StringFromLiteral("42"))
	first := socket.input.readByte()
	buffer := haxe__io__Bytes_alloc(4)
	read := socket.input.readBytes(buffer, 0, buffer.length)
	rest := socket.input.readAll().toString()
