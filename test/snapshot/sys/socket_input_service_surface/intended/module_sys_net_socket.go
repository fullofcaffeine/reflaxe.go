...
func sys__net__Socket_publicAddress(address *hxrt.SocketAddress) map[string]any {
	if address == nil {
		...
		hx_obj_11["host"] = sys__net__Host_fromIPv4(0)
		hx_obj_11["port"] = 0
		return hx_obj_11
	}
	hx_obj_12 := map[string]any{}
	hx_obj_12["host"] = sys__net__Host_fromIPv4(address.Host)
	hx_obj_12["port"] = address.Port
	return hx_obj_12
}
...
func sys__net__Socket_select_(read *hxrt.Array, write *hxrt.Array, others *hxrt.Array, timeout any) map[string]any {
	...
	readHandles := func() []*hxrt.SocketHandle {
		...
		_g1 := 0
		for _g1 < read.Len() {
			socket := func(hx_value_13 any) *sys__net__Socket {
				if hx_value_13 == nil {
	...
	writeHandles := func() []*hxrt.SocketHandle {
		...
		_g1_1 := 0
		for _g1_1 < write.Len() {
			socket_1 := func(hx_value_21 any) *sys__net__Socket {
				if hx_value_21 == nil {
	...
	otherHandles := func() []*hxrt.SocketHandle {
		...
		for _g1_2 < others.Len() {
			socket_2 := func(hx_value_29 any) *sys__net__Socket {
				if hx_value_29 == nil {
					var hx_zero_30 *sys__net__Socket
					return hx_zero_30
				}
				return hx_value_29.(*sys__net__Socket)
			}(others.Get(_g1_2))
			_g1_2 = int(int32((_g1_2 + 1)))
			_g_2.Push(socket_2.handle)
		}
		return func(hx_lambda_raw_32 []any) []*hxrt.SocketHandle {
			hx_lambda_out_33 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_32))
			for _, hx_lambda_item_34 := range hx_lambda_raw_32 {
				hx_lambda_out_33 = append(hx_lambda_out_33, func(hx_value_35 any) *hxrt.SocketHandle {
					if hx_value_35 == nil {
						var hx_zero_36 *hxrt.SocketHandle
						return hx_zero_36
					}
					return hx_value_35.(*hxrt.SocketHandle)
				}(hx_lambda_item_34))
			}
	...
	result := hxrt.SocketSelect(readHandles, writeHandles, otherHandles, func() float64 {
		var hx_if_37 float64
		if timeout == nil {
			hx_if_37 = 0.0
		} else {
			hx_if_37 = timeout.(float64)
		}
		return hx_if_37
	}(), (timeout != nil))
	hx_obj_38 := map[string]any{}
	hx_obj_38["read"] = sys__net__Socket_pick(read, result.Read)
	hx_obj_38["write"] = sys__net__Socket_pick(write, result.Write)
	hx_obj_38["others"] = sys__net__Socket_pick(others, result.Others)
	return hx_obj_38
}
