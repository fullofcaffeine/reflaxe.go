...
func sys__net__Socket_publicAddress(address *hxrt.SocketAddress) map[string]any {
	if address == nil {
		...
		hx_obj_120["host"] = sys__net__Host_fromIPv4(0)
		hx_obj_120["port"] = 0
		return hx_obj_120
	}
	hx_obj_121 := map[string]any{}
	hx_obj_121["host"] = sys__net__Host_fromIPv4(address.Host)
	hx_obj_121["port"] = address.Port
	return hx_obj_121
}
...
func sys__net__Socket_select_(read *hxrt.Array, write *hxrt.Array, others *hxrt.Array, timeout any) map[string]any {
	...
	readHandles := func() []*hxrt.SocketHandle {
		...
		_g1 := 0
		for _g1 < read.Len() {
			socket := func(hx_value_122 any) *sys__net__Socket {
				if hx_value_122 == nil {
	...
	writeHandles := func() []*hxrt.SocketHandle {
		...
		_g1_1 := 0
		for _g1_1 < write.Len() {
			socket_1 := func(hx_value_130 any) *sys__net__Socket {
				if hx_value_130 == nil {
					var hx_zero_131 *sys__net__Socket
					return hx_zero_131
				}
				return hx_value_130.(*sys__net__Socket)
			}(write.Get(_g1_1))
			_g1_1 = int(int32((_g1_1 + 1)))
			_g_1.Push(socket_1.handle)
		}
		return func(hx_lambda_raw_133 []any) []*hxrt.SocketHandle {
			hx_lambda_out_134 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_133))
			for _, hx_lambda_item_135 := range hx_lambda_raw_133 {
				hx_lambda_out_134 = append(hx_lambda_out_134, func(hx_value_136 any) *hxrt.SocketHandle {
					if hx_value_136 == nil {
						var hx_zero_137 *hxrt.SocketHandle
						return hx_zero_137
					}
					return hx_value_136.(*hxrt.SocketHandle)
				}(hx_lambda_item_135))
			}
	...
	otherHandles := func() []*hxrt.SocketHandle {
		...
		for _g1_2 < others.Len() {
			socket_2 := func(hx_value_138 any) *sys__net__Socket {
				if hx_value_138 == nil {
					var hx_zero_139 *sys__net__Socket
					return hx_zero_139
				}
				return hx_value_138.(*sys__net__Socket)
			}(others.Get(_g1_2))
			_g1_2 = int(int32((_g1_2 + 1)))
			_g_2.Push(socket_2.handle)
		}
		return func(hx_lambda_raw_141 []any) []*hxrt.SocketHandle {
			hx_lambda_out_142 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_141))
			for _, hx_lambda_item_143 := range hx_lambda_raw_141 {
				hx_lambda_out_142 = append(hx_lambda_out_142, func(hx_value_144 any) *hxrt.SocketHandle {
					if hx_value_144 == nil {
						var hx_zero_145 *hxrt.SocketHandle
						return hx_zero_145
					}
					return hx_value_144.(*hxrt.SocketHandle)
				}(hx_lambda_item_143))
			}
			return hx_lambda_out_142
		}(_g_2.Values())
	}()
	result := hxrt.SocketSelect(readHandles, writeHandles, otherHandles, func() float64 {
		var hx_if_146 float64
		if timeout == nil {
			hx_if_146 = 0.0
		} else {
			hx_if_146 = timeout.(float64)
		}
		return hx_if_146
	}(), (timeout != nil))
	hx_obj_147 := map[string]any{}
	hx_obj_147["read"] = sys__net__Socket_pick(read, result.Read)
	hx_obj_147["write"] = sys__net__Socket_pick(write, result.Write)
	hx_obj_147["others"] = sys__net__Socket_pick(others, result.Others)
	return hx_obj_147
}
