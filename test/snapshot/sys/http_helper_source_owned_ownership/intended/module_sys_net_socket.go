...
func sys__net__Socket_publicAddress(address *hxrt.SocketAddress) map[string]any {
	if address == nil {
		...
		hx_obj_16["host"] = sys__net__Host_fromIPv4(0)
		hx_obj_16["port"] = 0
		return hx_obj_16
	}
	hx_obj_17 := map[string]any{}
	hx_obj_17["host"] = sys__net__Host_fromIPv4(address.Host)
	hx_obj_17["port"] = address.Port
	return hx_obj_17
}
...
func sys__net__Socket_select_(read *hxrt.Array, write *hxrt.Array, others *hxrt.Array, timeout any) map[string]any {
	...
	readHandles := func() []*hxrt.SocketHandle {
		...
		_g1 := 0
		for _g1 < read.Len() {
			socket := func(hx_value_18 any) *sys__net__Socket {
				if hx_value_18 == nil {
					var hx_zero_19 *sys__net__Socket
					return hx_zero_19
				}
				return hx_value_18.(*sys__net__Socket)
			}(read.Get(_g1))
			_g1 = int(int32((_g1 + 1)))
			_g.Push(socket.handle)
		}
		return func(hx_lambda_raw_21 []any) []*hxrt.SocketHandle {
			hx_lambda_out_22 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_21))
			for _, hx_lambda_item_23 := range hx_lambda_raw_21 {
				hx_lambda_out_22 = append(hx_lambda_out_22, func(hx_value_24 any) *hxrt.SocketHandle {
					if hx_value_24 == nil {
						var hx_zero_25 *hxrt.SocketHandle
						return hx_zero_25
					}
					return hx_value_24.(*hxrt.SocketHandle)
				}(hx_lambda_item_23))
			}
			return hx_lambda_out_22
		}(_g.Values())
	}()
	writeHandles := func() []*hxrt.SocketHandle {
		...
		_g1_1 := 0
		for _g1_1 < write.Len() {
			socket_1 := func(hx_value_26 any) *sys__net__Socket {
				if hx_value_26 == nil {
					var hx_zero_27 *sys__net__Socket
					return hx_zero_27
				}
				return hx_value_26.(*sys__net__Socket)
			}(write.Get(_g1_1))
			_g1_1 = int(int32((_g1_1 + 1)))
			_g_1.Push(socket_1.handle)
		}
		return func(hx_lambda_raw_29 []any) []*hxrt.SocketHandle {
			hx_lambda_out_30 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_29))
			for _, hx_lambda_item_31 := range hx_lambda_raw_29 {
				hx_lambda_out_30 = append(hx_lambda_out_30, func(hx_value_32 any) *hxrt.SocketHandle {
					if hx_value_32 == nil {
						var hx_zero_33 *hxrt.SocketHandle
						return hx_zero_33
					}
					return hx_value_32.(*hxrt.SocketHandle)
				}(hx_lambda_item_31))
			}
	...
	otherHandles := func() []*hxrt.SocketHandle {
		...
		for _g1_2 < others.Len() {
			socket_2 := func(hx_value_34 any) *sys__net__Socket {
				if hx_value_34 == nil {
					var hx_zero_35 *sys__net__Socket
					return hx_zero_35
				}
				return hx_value_34.(*sys__net__Socket)
			}(others.Get(_g1_2))
			_g1_2 = int(int32((_g1_2 + 1)))
			_g_2.Push(socket_2.handle)
		}
		return func(hx_lambda_raw_37 []any) []*hxrt.SocketHandle {
			hx_lambda_out_38 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_37))
			for _, hx_lambda_item_39 := range hx_lambda_raw_37 {
				hx_lambda_out_38 = append(hx_lambda_out_38, func(hx_value_40 any) *hxrt.SocketHandle {
					if hx_value_40 == nil {
						var hx_zero_41 *hxrt.SocketHandle
						return hx_zero_41
					}
					return hx_value_40.(*hxrt.SocketHandle)
				}(hx_lambda_item_39))
			}
	...
	result := hxrt.SocketSelect(readHandles, writeHandles, otherHandles, func() float64 {
		var hx_if_42 float64
		if timeout == nil {
			hx_if_42 = 0.0
		} else {
			hx_if_42 = timeout.(float64)
		}
		return hx_if_42
	}(), (timeout != nil))
	hx_obj_43 := map[string]any{}
	hx_obj_43["read"] = sys__net__Socket_pick(read, result.Read)
	hx_obj_43["write"] = sys__net__Socket_pick(write, result.Write)
	hx_obj_43["others"] = sys__net__Socket_pick(others, result.Others)
	return hx_obj_43
}
