...
func sys__net__Socket_publicAddress(address *hxrt.SocketAddress) map[string]any {
	if address == nil {
		...
		hx_obj_47["host"] = sys__net__Host_fromIPv4(0)
		hx_obj_47["port"] = 0
		return hx_obj_47
	}
	hx_obj_48 := map[string]any{}
	hx_obj_48["host"] = sys__net__Host_fromIPv4(address.Host)
	hx_obj_48["port"] = address.Port
	return hx_obj_48
}
...
func sys__net__Socket_select_(read *hxrt.Array, write *hxrt.Array, others *hxrt.Array, timeout any) map[string]any {
	...
	readHandles := func() []*hxrt.SocketHandle {
		...
		_g1 := 0
		for _g1 < read.Len() {
			socket := func(hx_value_49 any) *sys__net__Socket {
				if hx_value_49 == nil {
					var hx_zero_50 *sys__net__Socket
					return hx_zero_50
				}
				return hx_value_49.(*sys__net__Socket)
			}(read.Get(_g1))
			_g1 = int(int32((_g1 + 1)))
			_g.Push(socket.handle)
		}
		return func(hx_lambda_raw_52 []any) []*hxrt.SocketHandle {
			hx_lambda_out_53 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_52))
			for _, hx_lambda_item_54 := range hx_lambda_raw_52 {
				hx_lambda_out_53 = append(hx_lambda_out_53, func(hx_value_55 any) *hxrt.SocketHandle {
					if hx_value_55 == nil {
						var hx_zero_56 *hxrt.SocketHandle
						return hx_zero_56
					}
					return hx_value_55.(*hxrt.SocketHandle)
				}(hx_lambda_item_54))
			}
			return hx_lambda_out_53
		}(_g.Values())
	}()
	writeHandles := func() []*hxrt.SocketHandle {
		...
		_g1_1 := 0
		for _g1_1 < write.Len() {
			socket_1 := func(hx_value_57 any) *sys__net__Socket {
				if hx_value_57 == nil {
					var hx_zero_58 *sys__net__Socket
					return hx_zero_58
				}
				return hx_value_57.(*sys__net__Socket)
			}(write.Get(_g1_1))
			_g1_1 = int(int32((_g1_1 + 1)))
			_g_1.Push(socket_1.handle)
		}
		return func(hx_lambda_raw_60 []any) []*hxrt.SocketHandle {
			hx_lambda_out_61 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_60))
			for _, hx_lambda_item_62 := range hx_lambda_raw_60 {
				hx_lambda_out_61 = append(hx_lambda_out_61, func(hx_value_63 any) *hxrt.SocketHandle {
					if hx_value_63 == nil {
						var hx_zero_64 *hxrt.SocketHandle
						return hx_zero_64
					}
					return hx_value_63.(*hxrt.SocketHandle)
				}(hx_lambda_item_62))
			}
			return hx_lambda_out_61
		}(_g_1.Values())
	}()
	otherHandles := func() []*hxrt.SocketHandle {
		...
		for _g1_2 < others.Len() {
			socket_2 := func(hx_value_65 any) *sys__net__Socket {
				if hx_value_65 == nil {
					var hx_zero_66 *sys__net__Socket
					return hx_zero_66
				}
				return hx_value_65.(*sys__net__Socket)
			}(others.Get(_g1_2))
			_g1_2 = int(int32((_g1_2 + 1)))
			_g_2.Push(socket_2.handle)
		}
		return func(hx_lambda_raw_68 []any) []*hxrt.SocketHandle {
			hx_lambda_out_69 := make([]*hxrt.SocketHandle, 0, len(hx_lambda_raw_68))
			for _, hx_lambda_item_70 := range hx_lambda_raw_68 {
				hx_lambda_out_69 = append(hx_lambda_out_69, func(hx_value_71 any) *hxrt.SocketHandle {
					if hx_value_71 == nil {
						var hx_zero_72 *hxrt.SocketHandle
						return hx_zero_72
					}
					return hx_value_71.(*hxrt.SocketHandle)
				}(hx_lambda_item_70))
			}
			return hx_lambda_out_69
		}(_g_2.Values())
	}()
	result := hxrt.SocketSelect(readHandles, writeHandles, otherHandles, func() float64 {
		var hx_if_73 float64
		if timeout == nil {
			hx_if_73 = 0.0
		} else {
			hx_if_73 = timeout.(float64)
		}
		return hx_if_73
	}(), (timeout != nil))
	hx_obj_74 := map[string]any{}
	hx_obj_74["read"] = sys__net__Socket_pick(read, result.Read)
	hx_obj_74["write"] = sys__net__Socket_pick(write, result.Write)
	hx_obj_74["others"] = sys__net__Socket_pick(others, result.Others)
	return hx_obj_74
}
