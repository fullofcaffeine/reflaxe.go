...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_8 := false
		var hx_try_value_9 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
			...
			case *haxe__io__Eof:
				...
				hx_try_value_9 = partial
				hx_try_return_8 = true
				return
			default:
...
func haxe__io__GoIoHelpers_outputWriteInput(self haxe__io__Output, i haxe__io__Input, bufsize int) {
	...
	for true {
		...
		hxrt.TryCatch(func() {
			...
			switch hx_typed_13 := hx_caught_12.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_13
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_12)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_14 *haxe__io__Bytes
	if encoding == nil {
		hx_if_14 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_14 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_14
