...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_67 := false
		var hx_try_value_68 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
		...
		}, func(hx_caught_69 any) {
			switch hx_typed_70 := hx_caught_69.(type) {
			case *haxe__io__Eof:
				e := hx_typed_70
				...
				hx_try_value_68 = partial
				hx_try_return_67 = true
				return
			default:
				hxrt.Throw(hx_caught_69)
			}
		})
		if hx_try_return_67 {
			return hx_try_value_68
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteInput(self haxe__io__Output, i haxe__io__Input, bufsize int) {
	...
	for true {
		...
		}, func(hx_caught_71 any) {
			switch hx_typed_72 := hx_caught_71.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_72
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_71)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_73 *haxe__io__Bytes
	if encoding == nil {
		hx_if_73 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_73 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_73
