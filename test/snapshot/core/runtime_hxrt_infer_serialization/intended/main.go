...
func main() {
	encoded := haxe__Serializer_run(func() map[string]any {
		hx_obj_1 := map[string]any{}
		hx_obj_1["value"] = 1
		return hx_obj_1
	}())
	var v any = any((hxrt.StringLengthStringPtr(encoded) > 0))
	hxrt.Println(v)
}
...
func haxe__GoSerializationBridge_hasSerializeHook(value any) bool {
	if value == nil {
		return false
	}
	if _, ok := value.(interface{ hxSerialize(*haxe__Serializer) }); ok {
		return true
	}
	if _, ok := value.(interface{ HxSerialize(*haxe__Serializer) }); ok {
		return true
...
func haxe__GoSerializationBridge_callSerializeHook(value any, serializer *haxe__Serializer) bool {
	if hook, ok := value.(interface{ hxSerialize(*haxe__Serializer) }); ok {
		hook.hxSerialize(serializer)
		return true
	}
	if hook, ok := value.(interface{ HxSerialize(*haxe__Serializer) }); ok {
		hook.HxSerialize(serializer)
