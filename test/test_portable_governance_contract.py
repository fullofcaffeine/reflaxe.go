...
class PortableGovernanceContractTest(unittest.TestCase):
    ...
    def test_known_gaps_has_release_checkable_production_scoreboard(self) -> None:
        ...
        self.assertIn("durable evidence links and commands, not retired tracker IDs", release_checklist)
        self.assertIn("docs/multi-package-output-evaluation.md#measurable-production-reopen-triggers", known_gaps)
        self.assertIn("docs/performance-budget-policy.md", known_gaps)
        self.assertIn("test/run-goextern-fixtures.py", known_gaps)
        self.assertNotIn("Current GA hardening beads", known_gaps)
        self.assertNotIn("haxe.go-1bo0.2", known_gaps)
        self.assertNotIn("haxe.go-1bo0.7", known_gaps)
