...
- `haxe_go-vfp.8.7.6` moved the supported root `Sys` API into canonical `std/go/_std/Sys.hx` over typed `std/hxrt` capabilities. Environment-map construction, fallbacks, aliases, and standard-stream wrapping no longer live in `GoCompiler`; separate selective snapshots prove root Sys and `sys.io.Process` do not pull each other's runtime slices.
- `haxe_go-vfp.8.7.7` moved the complete `sys.io.Process` API and its `haxe.io` stream wrappers into canonical `std/go/_std/sys/io/Process.hx`. Typed opaque handles in `std/hxrt/process` preserve native spawn, pipe, wait, signal, and close capabilities without `Dynamic`/`Any`; the former Process emitter and its 134 raw sites left `GoCompiler`.
