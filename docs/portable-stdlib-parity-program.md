...
- `haxe.go-14as.57` closed the remaining direct `haxe.rtti.*` reflection tranche after the HTTP half was split out. `haxe.rtti.Meta`, `haxe.rtti.Rtti`, `haxe.rtti.CType`, and `haxe.rtti.XmlParser` now have parity evidence through `haxe_rtti_direct_contract` and `stdlib/haxe_rtti_direct`. Ownership stays mixed on purpose: public parser/type logic lives in `std/haxe/rtti/*.cross.hx`, while the backend still owns the narrow class-token `__meta__` / `__rtti` contract plus anonymous-record array-field mutation lowering under that staged surface.
- `haxe.go-14as.15` closed the direct `haxe.io` misc tranche. `haxe.io.BufferInput`, `haxe.io.BytesData`, `haxe.io.Encoding`, `haxe.io.Eof`, `haxe.io.Error`, `haxe.io.FPHelper`, `haxe.io.Mime`, `haxe.io.Scheme`, and `haxe.io.StringInput` now have parity evidence through `haxe_io_misc_contract` and `stdlib/haxe_io_misc_direct`. Ownership is explicit instead of hand-waved as one blob: `FPHelper` moved to staged std (`std/haxe/io/FPHelper.cross.hx`), `Mime` / `Scheme` stay upstream source-owned abstractions, and the base IO hierarchy still keeps `StringInput`, `BufferInput`, `Encoding`, `Eof`, and `Error` in compiler-owned shims because their type shapes and inherited helper wiring remain representation-sensitive on Go.
- `haxe.go-14as.17` closed the direct `sys.db` + `sys.io` tranche. `sys.db.Connection`, `sys.db.ResultSet`, `sys.db.Mysql`, `sys.db.Sqlite`, `sys.io.FileInput`, `sys.io.FileOutput`, and `sys.io.FileSeek` now have parity evidence through `sys_db_io_contract` and `stdlib/sys_db_io_direct`. Ownership is explicit instead of muddled: `sys.db` stays upstream/public source ownership, while `sys.io.File*` now uses compiler-owned wrapper types over a real `runtime/hxrt/sys.go` file-handle runtime for open/read/write/seek/tell/eof instead of compile-only stubs.
...
- `haxe.go-14as.40` kept native stack capture out of the portable semantic baseline and reduced it to a future explicit Go diagnostic capability. See `docs/spikes/native-stack-capture-contract.md`.
