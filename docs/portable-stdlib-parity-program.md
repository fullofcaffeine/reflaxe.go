...
- `haxe.go-14as.57` now owns the remaining direct `haxe.rtti.*` reflection tranche after the HTTP half was split out. The narrow backend closure is now in place for class-token `__rtti` / `__meta__` lookup plus anonymous-record array-field mutation on the existing carrier; the remaining work is the public staged-stdlib parity closeout for `Meta`, `Rtti`, `CType`, and `XmlParser`.
