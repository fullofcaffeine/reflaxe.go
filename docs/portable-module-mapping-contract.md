...
| `Lambda` | `mixed` (`haxe_source` + exact representation intrinsics) | `std/go/_std/Lambda.hx` owns traversal, comparison, early exit, allocation, filtering, mapping, and folding | `GoLambdaIterableLowering.hx` plus `lowerLambdaSourceCallAdapter` wrap only Go-invariant iterable, callback, and erased-result shapes for the eight migrated helpers | `lambda_generic_iterable_count_empty_contract`, `lambda_iter_array_list_contract`, `lambda_iter_generic_iterable_contract`, `lambda_list_contract`; source migration `haxe_go-vfp.8.7.17`, remaining API/carrier audit `haxe_go-vfp.8.7.18` |
