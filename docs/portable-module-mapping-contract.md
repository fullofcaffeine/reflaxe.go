...
| `Std` | `mixed` (exact `compiler_intrinsic` primitives + two explicit `compiler_migration` calls + runtime helpers) | No compiler-emitted `Std` type exists. Exact type/representation operations are individually registered; `Std.parseInt` remains a visible migration call rather than a nominal shim-group dependency. | `runtime/hxrt/string.go`, `runtime/hxrt/exception.go`, core helpers | `exception_api_contract`, `std_is_of_type_contract`, `std_is_of_type_runtime_core_abstract_contract`, `typed_nil_dynamic_string_contract`; source-ownership follow-up `haxe_go-vfp.8.7.22` |
