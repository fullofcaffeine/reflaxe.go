...
| `Sys` | `mixed` (`compiler wrapper` + `runtime_binding`) | `src/reflaxe/go/GoCompiler.hx` (`lowerSysStdlibShimDecls`) | `runtime/hxrt/sys.go`; portable `putEnv` intentionally discards the retained native error for Haxe 4.3.7 eval parity, while `Sys.sleep` delegates seconds-to-`time.Duration` conversion to `hxrt.SysSleep` | `sys_io_roundtrip`, `file_error_semantics_contract`, `sys_sleep_contract`, `sys/sys_sleep_portable` |
