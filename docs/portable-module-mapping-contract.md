...
| `Sys` | `mixed` (`haxe_source` + `runtime_binding`, with one compile-time unsupported diagnostic) | `std/go/_std/Sys.hx` with typed bindings in `std/hxrt/sys` and `std/hxrt/fs` | `runtime/hxrt/sys.go` for root process capabilities and `runtime/hxrt/file.go` for non-owning standard-stream handles; the complete root surface is specified in [Portable root `Sys` contract](portable-sys-contract.md). Portable `putEnv` discards the retained native error for Haxe 4.3.7 eval parity; `setTimeLocale` reports `false`; `cpuTime` fails at Haxe compile time. | `root_sys_contract`, `root_sys_portable_contract`, `sys_io_roundtrip`, `file_error_semantics_contract`, `sys_sleep_contract`, `sys/root_sys_portable`, `core/runtime_hxrt_infer_sys`, `negative/sys_cpu_time_unsupported` |
...
| `sys.io.Process` | `runtime_binding` | `src/reflaxe/go/GoCompiler.hx` (`lowerProcessStdlibShimDecls` type/error/stream adapters) | `runtime/hxrt/process.go` (`NewProcess`, `Process`, `ProcessInput`, `ProcessOutput`); startup/pipe/read/wait failures, EOF, and exit status remain distinct. This is the remaining compiler-owned migration in `haxe_go-vfp.8.7.7`. | `process_echo_contract`, `process_error_semantics_contract`, `sys/process_error_semantics`, `core/runtime_hxrt_infer_process` |
