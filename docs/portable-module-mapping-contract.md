...
| `sys.ssl` direct surfaces (`Certificate`, `Digest`, `DigestAlgorithm`, `Key`, `Socket`) | `mixed` | `std/sys/ssl/*.cross.hx` for the public API; `DigestAlgorithm` is fully source-owned | `runtime/hxrt/ssl.go` for certificate parsing, key parsing, digest/sign/verify, and TLS socket dial/listen/handshake helpers beneath the public wrappers | `stdlib/sys_ssl_leaf_direct`, `stdlib/sys_ssl_socket_direct`, `stdlib/sys_ssl_socket_sni_unimplemented`, `semantic_diff/sys_net_address_ssl_digest_algorithm_contract`, `stdlib/sys_net_address_ssl_digest_algorithm_direct` |
| `sys.thread` primitive direct surfaces (`Condition`, `Deque`, `IThreadPool`, `Lock`, `Mutex`, `NoEventLoopException`, `Semaphore`, `ThreadPoolException`, `Tls`) | `mixed` | `std/sys/thread/*.cross.hx` for the public API and source-owned queue/storage helpers | `runtime/hxrt/thread.go` for blocking locks, mutexes, conditions, semaphores, and thread-id lookup beneath the staged wrappers | `semantic_diff/sys_thread_primitives_contract`, `stdlib/sys_thread_primitives_direct` |
