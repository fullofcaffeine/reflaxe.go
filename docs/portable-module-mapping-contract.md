...
| `haxe.ds.List` | `haxe_source` | `std/go/_std/haxe/ds/List.hx`; its private array carrier is temporary compatibility for collection adapters tracked by `haxe_go-vfp.8.7.17` | None beyond ordinary array/string primitives | `ds_maps_list_contract`, `list_std_contract`, `lambda_list_contract`; source migration `haxe_go-vfp.8.7.10` |
