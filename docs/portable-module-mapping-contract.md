...
| base `haxe.io` (`Bytes`, buffers, `Input`, `Output`, encoding, EOF, error) | `mixed` (`haxe_source` + typed representation capabilities) | Canonical overrides under `std/go/_std/haxe/io` own every public type, stream loop, bounds/error rule, endian operation, RawNative policy, alias rule, and cache invalidation contract | `std/hxrt/io` exposes only opaque `ByteView`, allocation/conversion/copy, UTF codec, and IEEE-754 bit capabilities backed by `runtime/hxrt/bytes.go`; no `io` compiler group or generated public IO authority remains | bytes/IO/serializer/sys-IO semantic-diff, generated snapshots, strict sweep, direct runtime/perf gates; migration `haxe_go-vfp.8.7.11`; [ownership contract](haxe-io-ownership.md) |
