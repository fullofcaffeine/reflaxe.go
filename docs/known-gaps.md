...
| Advanced Go extern interop | typed native facade | Ship `@:go.import` / `@:go.name` / `@:go.receiver`, exact exported fields and zero values for supported Go structs, single `(T, error)` support through `go.Result<T>`, and generated tuple carrier wrappers for supported multi-return APIs. Keep complex/unsupported shapes behind typed facade wrappers. | [Go extern generator](goextern.md), `tools/goextern/main_test.go`, `test/run-goextern-fixtures.py`, `test/snapshot/go_native/extern_struct_zero_value`, `test/snapshot/go_native/extern_tuple_return`, `--dynamic-report` output. | Users hit common Go APIs whose callback/generic/unsafe, map-field, width-changing, or cross-package shapes still fall back or are omitted with evidence; file a new scoped Bead for a typed generator rule or documented facade pattern, not raw `__go__` in app code. |
