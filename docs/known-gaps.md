...
| Advanced Go extern interop | typed extern boundary | Ship precise supported named types across package boundaries. Keep exact exported fields and zero values for supported Go structs. Support single `(T, error)` results through `go.Result<T>`. Generate tuple carriers for supported multi-return APIs. Keep complex shapes behind precise user externs or narrow typed adapters. | [Go extern generator](goextern.md), `tools/goextern/main_test.go`, `test/test_goextern_cross_package_e2e.py`, `test/run-goextern-fixtures.py`, `test/snapshot/go_native/extern_struct_zero_value`, `test/snapshot/go_native/extern_tuple_return`, `--dynamic-report` output. | Users hit common Go APIs whose callback, generic, unsafe, map-field, width-changing, or unsupported container shapes still fall back or are omitted. File a scoped Bead with evidence for a typed generator rule. Do not use raw `__go__` in app code. |
...
- Typed extern metadata (`@:go.import`, `@:go.name`, `@:go.receiver`, `@:go.struct`, `@:go.valueType`) is supported. Generated bindings preserve named struct and scalar value parameters and single or tuple results while keeping the Haxe view pointer-backed. Advanced Go shapes may still require façade wrappers.
