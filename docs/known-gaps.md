...
- Reason: the narrow backend blocker is now isolated. Go has the class-token `__rtti` / `__meta__` lookup contract and safe anonymous-record array-field mutation on the existing carrier, but the public `haxe.rtti.*` staged-stdlib parity closeout still remains under `haxe.go-14as.57`.
