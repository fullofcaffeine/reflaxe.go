...
- Standard-stream carriers are non-owning: `close()` cannot close the process descriptor, and stdout/stderr flush avoids `os.File.Sync` failures on pipes and terminals. Ordinary `sys.io.File` carriers remain owning and sync-on-flush.
