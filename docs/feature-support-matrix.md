...
| Surface | Status | Evidence (snapshot IDs) |
| --- | --- | --- |
| Channels and goroutines | Supported (real goroutine/channel/select lowering; non-metal now applies typed recv/recvOr/tryRecv assertion bridging for `go.Chan<T>` reads; `metal` adds concrete typed shim lanes) | `go_native/channel_basic`, `go_native/channel_try_recv`, `go_native/channel_select_handshake`, `go_native/channel_metal_monomorph`, `go_native/goroutine_smoke` |
