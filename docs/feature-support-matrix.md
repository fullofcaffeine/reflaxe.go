...
| `sys.net.Socket` | `semantic-diff` + `snapshot` + direct race/cross-build | `socket_loopback_contract`, `socket_advanced_contract`, `socket_server_lifecycle_contract`, `sys/socket_input_service_surface`, `sys/socket_server_lifecycle_contract`, `core/runtime_hxrt_infer_socket`, partial-I/O/peer-close/deadline/lifecycle cases in `runtime/hxrt/socket_test.go`, Linux backlog evidence in `runtime/hxrt/socket_listener_linux_test.go`, `test_socket_runtime_cross_build.py` |
...
| Typed Go file build constraints | Supported for non-main Haxe modules (`@:goBuildConstraint`) | `core/typed_go_build_constraints`, `negative/go_build_constraint_invalid`, `negative/go_build_constraint_main`, `negative/go_build_constraint_module_conflict`, `negative/go_build_constraint_repeated` |
...
| Extern metadata mapping | Supported (`@:go.import`/`@:go.name`/`@:go.receiver`, `@:go.struct` zero values and exact selectors for supported exported fields, `@:go.valueType` named scalars, generated named-value argument and single/tuple result ABI adaptation, imported extern `String` call and field normalization via `hxrt.StdString`, `@:go.valueError` mapping for `(T,error)` extern calls to `go.Result<T>`, and `@:go.tupleReturn` mapping for generated multi-return carrier classes) | `go_native/extern_metadata_mapping`, `go_native/extern_struct_zero_value`, `go_native/extern_struct_value_abi`, `go_native/extern_named_scalar_value_abi`, `go_native/extern_value_error_result`, `go_native/extern_tuple_return` |
...
- `runtime/hxrt/socket_ssl.go` owns only native TLS client setup, deferred
  listener wrapping, handshake, peer-certificate access, and synchronized SNI
  selection. TLS bind retains its typed configuration on the bound TCP handle;
  inherited `listen(connections)` performs the real listen transition before
  wrapping it. Certificate/key/digest primitives remain in `ssl.go`; SSL leaf
  users therefore do not select network transport.
