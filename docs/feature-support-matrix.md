...
| `StringBuf` + `DateTools` + `Lambda` core subset (`add`/`addChar`/`addSub`, `DateTools.delta` with duration helpers, staged-std `DateTools.format` strftime subset plus `getMonthDays`/`parse`/`make`, source-owned `Lambda.filter`/`map`/`fold`/`has`/`exists`/`iter` over `Array<T>`/`haxe.ds.List<T>`, generic-`Iterable<T>` support for `Lambda.count` including its optional predicate, `empty`/`exists`/`has`/`filter`/`map`/`fold`/`iter`, function-value `Lambda.map`/`fold` calls over generic and array carriers, and upstream-inline `Lambda.mapi` over arrays; remaining full-API carrier parity is tracked by `haxe_go-vfp.8.7.18`) | `semantic-diff` | `stringbuf_datetools_lambda_contract`, `datetools_cross_std_contract`, `lambda_list_contract`, `lambda_generic_iterable_count_empty_contract`, `lambda_iter_array_list_contract`, `lambda_iter_generic_iterable_contract` |
