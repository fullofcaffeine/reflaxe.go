...
- Canonical staged `std/go/_std/Sys.hx` owns root `Sys`, including environment-map construction, aliases, fallbacks, and standard-stream construction. Narrow typed bindings in `std/hxrt/sys` and `std/hxrt/fs` cross into native capabilities. Compiler-generated declarations now cover only the still-open `sys.io.Process` migration.
