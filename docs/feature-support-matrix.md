...
- `haxe.go-dt4s`: re-opened direct `haxe.EntryPoint` / `haxe.MainLoop` / `haxe.Timer` support after CI exposed broken generated Go from the old source-owned inclusion path. Those modules now stay explicit compile-only blockers until the event-loop surface is real.
- `haxe.go-14as.14` to `haxe.go-14as.19`, `haxe.go-14as.25` to `haxe.go-14as.27`, `haxe.go-14as.43` to `haxe.go-14as.47`, and `haxe.go-dt4s`: dated blocker families for the remaining compile-only portable tranches (`haxe_http_rtti`, `haxe_io_misc`, `haxe_io_typed_arrays`, `sys_db_io`, `sys_net_ssl`, `sys_thread`, `haxe_misc_symbols`, `haxe_misc_abstractions`, `haxe_misc_enum_helpers`, `haxe_exceptions_direct`, `haxe_ds_hashmap_direct`, `haxe_ds_sort_helpers`, `haxe_ds_source_owned_collections`, `haxe_ds_weakmap`, `haxe_event_loop_direct`).
