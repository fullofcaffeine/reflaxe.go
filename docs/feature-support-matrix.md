...
- Native behavior is split by capability across `runtime/hxrt/sys.go`, `runtime/hxrt/file.go`, and `runtime/hxrt/process.go`:
...
- Compiler-generated `sys` declarations now cover only the still-open root `Sys` and `sys.io.Process` migrations. Root standard-stream adapters construct the staged `FileInput` / `FileOutput` wrappers; `GoCompiler` no longer synthesizes File types, handle maps, seek mappers, static APIs, or stream methods.
...
- The remaining root surface is explicit rather than symbol-dependent: print/clock/cwd/program-path methods and all three standard streams have typed adapters; `setTimeLocale` reports `false`; deprecated `executablePath` aliases `programPath`; and `Sys.cpuTime` fails during Haxe compilation because Go has no portable process CPU clock. See [Portable root `Sys` contract](portable-sys-contract.md).
