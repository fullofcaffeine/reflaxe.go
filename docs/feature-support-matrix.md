...
| Surface | Status | Evidence (snapshot IDs) |
| --- | --- | --- |
| Channels and goroutines | Supported (real goroutine/channel/select lowering; non-metal applies typed recv/recvOr/tryRecv assertion bridging for `go.Chan<T>` reads; typed deterministic `go.Select` helper API is available for receive/send branching; `metal` adds concrete typed shim lanes) | `go_native/channel_basic`, `go_native/channel_try_recv`, `go_native/channel_select_handshake`, `go_native/channel_metal_monomorph`, `go_native/goroutine_smoke`, `go_native/select_helpers` |
