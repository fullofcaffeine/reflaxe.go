...
- `haxe.go-14as.28`: closed stack/main-loop `haxe.misc` triage by moving the tranche into explicit target-sensitive snapshot coverage. Added staged std fallbacks for `haxe.CallStack` and `haxe.NativeStackTrace`, source-owned inclusion for direct `haxe.EntryPoint` / `haxe.MainLoop` / `haxe.Timer` usage, and direct snapshot coverage in `stdlib/haxe_stack_loop_target_sensitive`.
