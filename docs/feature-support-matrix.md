...
| `StringBuf` + `DateTools` + `Lambda` core subset (`add`/`addChar`, `DateTools.delta` with duration helpers, iterable `Lambda.map`) | `semantic-diff` | `stringbuf_datetools_lambda_contract` |
