...
- Direct `haxe.EntryPoint` / `haxe.MainLoop` / `haxe.Timer` usage remains an explicit unsupported surface on Go. That is no longer because `sys.thread.Thread` / `sys.thread.EventLoop` are missing; it is because `haxe.MainLoop` still is not integrated with the new runtime-backed thread/event-loop contract. Follow-up work is tracked in `haxe.go-14as.69`.
