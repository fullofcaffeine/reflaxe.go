...
| `StringBuf` + `DateTools` + `Lambda` core subset (`add`/`addChar`/`addSub`, `DateTools.delta` with duration helpers and `%Y-%m-%d %H:%M:%S` formatting, `Lambda.filter`/`map`/`fold`/`has`/`exists` over `Array<T>`/`haxe.ds.List<T>`, generic-`Iterable<T>` fallback for `Lambda.count`/`Lambda.empty`/`Lambda.exists`/`Lambda.has`/`Lambda.filter`/`Lambda.map`/`Lambda.fold` including function-value `Lambda.map` call-sites; generic `Iterable<T>` inputs still fail with deterministic compile-time diagnostics for `Lambda.iter`) | `semantic-diff` | `stringbuf_datetools_lambda_contract`, `lambda_list_contract`, `lambda_generic_iterable_count_empty_contract`, `negative/lambda_generic_iterable_unsupported` |
