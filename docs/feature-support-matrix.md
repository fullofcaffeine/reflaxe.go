...
- `haxe.go-14as.57`: closed the direct `haxe.rtti.*` reflection tranche. `haxe.rtti.Meta`, `haxe.rtti.Rtti`, `haxe.rtti.CType`, and `haxe.rtti.XmlParser` now have staged-stdlib baseline support with mixed ownership: public parser/type logic lives in `std/haxe/rtti/*.cross.hx`, while the backend owns the narrow class-token `__meta__` / `__rtti` contract plus anonymous-record array-field mutation lowering underneath. Evidence: `haxe_rtti_direct_contract` and `stdlib/haxe_rtti_direct`.
...
- `haxe.go-14as.19` first wave: direct `sys.thread` primitives are now covered. `Condition`, `Deque`, `IThreadPool`, `Lock`, `Mutex`, `NoEventLoopException`, `Semaphore`, `ThreadPoolException`, and `Tls` now have parity evidence through `sys_thread_primitives_contract` and `stdlib/sys_thread_primitives_direct`. The public API stays in staged std under `std/sys/thread/*.cross.hx`, while the blocking primitives themselves live in `runtime/hxrt/thread.go`.
