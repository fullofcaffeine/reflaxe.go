...
| Extern metadata mapping | Supported (`@:go.import`/`@:go.name`/`@:go.receiver`, `@:go.struct` zero values and exact selectors for supported exported fields, generated struct value-argument and single/tuple value-result ABI adaptation, imported extern `String` call and field normalization via `hxrt.StdString`, `@:go.valueError` mapping for `(T,error)` extern calls to `go.Result<T>`, and `@:go.tupleReturn` mapping for generated multi-return carrier classes) | `go_native/extern_metadata_mapping`, `go_native/extern_struct_zero_value`, `go_native/extern_struct_value_abi`, `go_native/extern_value_error_result`, `go_native/extern_tuple_return` |
