...
| Symbol | Granularity | State | Release admitted | Evidence | Qualification |
| --- | --- | --- | --- | --- | --- |
| `haxe.Http request callbacks` | `operation` | `experimental` | no | `semantic:http`, `runtime:http-regressions`, `snapshot:http-selective-runtime`, `bead:network-audit` | Canonical staged haxe.Http/sys.Http now owns request and callback policy over typed opaque HTTP/socket handles. Loopback semantic contracts plus direct runtime tests cover timeouts, response/body cleanup, proxy formatting, truncated-body status preservation, and socket consumption; broad cross-platform cancellation and hostile-peer admission remain open. |
| `sys.Http request callbacks` | `operation` | `experimental` | no | `semantic:http`, `runtime:http-regressions`, `snapshot:http-selective-runtime`, `bead:network-audit` | Canonical staged haxe.Http/sys.Http now owns request and callback policy over typed opaque HTTP/socket handles. Loopback semantic contracts plus direct runtime tests cover timeouts, response/body cleanup, proxy formatting, truncated-body status preservation, and socket consumption; broad cross-platform cancellation and hostile-peer admission remain open. |
