...
| `sys.net.Socket.shutdown` | `operation` | `experimental` | no | `semantic:socket`, `runtime:socket-regressions`, `bead:network-audit` | Deterministic fixtures exercise current TCP shutdown and fast-send behavior, but one-sided TLS shutdown and applying TCP_NODELAY through TLS remain unresolved. These members are intentionally separate from the real-readiness candidate so readiness evidence cannot imply their admission. |
...
| `sys.ssl.Socket` | `surface` | `experimental` | no | `snapshot:socket-tls`, `snapshot:socket-sni`, `runtime:socket-regressions`, `runtime:socket-cross-build`, `bead:network-audit` | Deterministic local-CA runtime evidence proves that direct TLS dials a resolved IPv4 address while retaining the original logical hostname for default certificate verification and SNI; mismatch failure and explicit setHostname override are covered. Generated-Haxe loopback snapshots exercise the same default endpoint path. TLS remains experimental because inherited control semantics, public certificate stores, hostile peers, and runtime behavior outside Linux/amd64 are not admitted; Windows evidence is compile-only. |
### `go-native`
