...
| `sys.thread.Tls` | `surface` | `experimental` | no | `semantic:thread-primitives`, `contract:concurrency` | Get, set, clear, and synchronization behavior is tested, but completed-thread value reclamation and detached-goroutine attachment are not yet bounded. |
...
| Bead | Scope | Blocks |
| --- | --- | --- |
| `haxe_go-vfp.10.7` | Completed-thread TLS reclamation and compiler-owned detached-goroutine identity cleanup | portable-concurrency/tls-lifecycle |
