...
| `sys` | root `Sys` | 0 (compiler ownership retired) | Semantic-diff | Migrated (canonical staged std + typed runtime capabilities) | `std/go/_std/Sys.hx` owns the public API, map construction, fallbacks, aliases, and standard-stream wrapping. Typed `std/hxrt` bindings expose capabilities in `runtime/hxrt/sys.go`, `file.go`, and the baseline print slice. Only the honest compile-time `cpuTime` rejection remains in the compiler. | `haxe.go-7zy.11`, `haxe_go-vfp.8.7.1`, `haxe_go-vfp.8.7.2`, `haxe_go-vfp.8.7.6` |
| `process` | `sys.io.Process` | 320 current span (134 `GoRaw` sites) | Semantic-diff | Keep isolated adapters pending the Process source migration | Native child-process behavior lives in `runtime/hxrt/process.go`; `lowerProcessStdlibShimDecls` still owns generated carrier, stream, error, EOF, and status adapters. Root Sys and File no longer enter this group. | `haxe.go-7zy.11`, `haxe_go-vfp.8.7.7` |
...
| `SDR-002` | `process` (`sys.io.Process`) | Keep the remaining compiler carrier/error/stream adapters isolated in `lowerProcessStdlibShimDecls` while native child-process behavior stays in `runtime/hxrt/process.go`; migrate the public surface separately under `haxe_go-vfp.8.7.7`. | Keep the former combined Sys/Process group, direct externs, canonical `std/go/_std` | Process semantic-diff contracts, selective `core/runtime_hxrt_infer_process`, and the historical migration log from `haxe.go-7zy.11` |
...
## Ownership Boundary (Post `haxe_go-vfp.8.7.6`)
...
- `std/go/_std/sys/io` owns the File public API and stream semantics, and `std/go/_std/Sys.hx` owns root `Sys`. Neither family has compiler-generated declarations or semantic branches.
- `lowerProcessStdlibShimDecls` is the isolated remaining adapter group for `sys.io.Process`; OS child-process behavior belongs in `runtime/hxrt/process.go`, and its source migration is `haxe_go-vfp.8.7.7`.
