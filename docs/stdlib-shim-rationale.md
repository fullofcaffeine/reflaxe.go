...
| `net_socket` (`sys.net.Host`, `sys.net.Socket`) | `GoCompiler` `net_socket` shim group | Keep compiler-owned | Deadline/select/shutdown/readiness behavior is still target-specific and tightly contract-tested. Extracting raw leaf helpers now would add churn without reducing real ownership complexity. | Keep; revisit only with a typed replacement path that preserves readiness semantics |
