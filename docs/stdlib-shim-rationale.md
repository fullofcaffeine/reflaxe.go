...
| `SDR-014` | portable exception carrier and `sys.thread` process-lifecycle boundary | Keep the public API in staged std, require portable runtime validation failures to use `Throw`, own worker state/recovery/reporting in `hxrt`, and let the compiler add only a feature-gated generated-main foreground drain. Recover only explicit Haxe exception carriers; keep `go.Go.spawn` and typed Go panics bare and native. | Treat every panic as a Haxe value, use raw panic for portable validation failures, let uncaught Haxe throws crash the Go process, join all native goroutines, or make portable workers daemon-like | `core/portable_runtime_failure_haxe_catch`, `stdlib/sys_thread_uncaught_exception`, `go_native/native_panic_not_haxe_catch`, `go_native/goroutine_native_panic`, `go_native/goroutine_native_shutdown`, direct `runtime/hxrt` tests, and Haxe 4.3.7 interpreter probe recorded in `haxe_go-vfp.10.1` |
