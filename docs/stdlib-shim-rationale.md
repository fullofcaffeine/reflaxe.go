...
| `stdlib_symbols` | `Std`, `Reflect`, Unicode, Option, Type metadata | 441 | Semantic-diff | Migration required except exact registered metadata/representation primitives | Unicode algorithms and runtime reflection remain source/runtime migration work. Crypto, XML, zip, Date, and Math have left this group. Only separately registered type metadata, type-test, string representation, exception carrier, and Rest primitives are admitted. | `haxe_go-vfp.8.7.15` |
...
| `SDR-008` | `stdlib_symbols` library surfaces | Continue completed staged migrations and retire the remaining monolithic Reflect/Unicode/Option block. Crypto, XML, zip, Date, and Math have already moved to their source/runtime owners. Only exact metadata and representation primitives in the intrinsic registry may remain. | Preserve the mixed blob, extern-only wrappers, staged source plus narrow runtime helpers | Sibling-target precedent, existing local migrations, and `haxe_go-vfp.8.7.15` |
