...
| retired `stdlib_symbols` | former nominal `Std` carrier, `haxe.ds.Option`, and unrelated classifier/planner selectors | 0 | Snapshot + semantic-diff + strict sweep | Retired | `Option` now follows ordinary source enum lowering. The empty `Std` carrier and false `BalancedTree`, `Path`, Template, and SSL selectors are gone. Type, Reflect, generated-method metadata, type tests, string/exception representations, and Rest remain only as separately registered exact capabilities. `Std.parseInt` and `haxe.Log.trace` are visible migration calls with their own follow-up, not a replacement group. | retirement `haxe_go-vfp.8.7.15.7`; Std/Log follow-up `haxe_go-vfp.8.7.22` |
| retired `regex_serializer` | `EReg`, serialization token/cache/traversal policy | 0 behavior-heavy compiler declarations | Semantic-diff + snapshot + direct runtime + checkptr | Migrated (canonical staged std + typed runtime capabilities) | Staged Haxe owns regex state/policy and the full serialization algorithms. `regex.go` owns RE2 execution; `serialization.go` owns only reflected field access and hidden-self repair. Existing Type metadata owns class/enum lookup and construction. | `haxe_go-vfp.8.7.13` |
| `serialization_source_bridge` | package-private `hxSerialize` / `hxUnserialize` hooks and structural `resolveClass` / `resolveEnum` callbacks | one 81-line typed declaration emitter | Semantic-diff + snapshot + direct runtime + checkptr | Approved exact representation adapter | The bridge is selected only when staged Serializer or Unserializer is reachable. It invokes generated same-package methods that external `hxrt` cannot name; it owns no token parsing, traversal, caches, reflection policy, metadata tables, or construction. | `haxe_go-vfp.8.7.13`; explicit group boundary `haxe_go-vfp.8.7.15.7` |
...
| `SDR-006` | retired `regex_serializer` plus exact `serialization_source_bridge` | Keep the complete public algorithms in canonical staged Haxe. Cross only typed RE2 and reflected field capabilities into feature-sliced `hxrt`; reuse approved Type metadata for class/enum lookup and construction. Select the separately registered same-package invocation bridge only when staged Serializer or Unserializer is reachable. | Keep the mixed compiler emitter, move its blob unchanged into `hxrt`, generate serializer-specific duplicate metadata tables, emit the bridge unconditionally, or hide it under an unrelated compatibility group | Thirteen regex/serializer semantic-diff contracts, two selective-runtime snapshots, direct runtime tests, checkptr/security gates, intrinsic registry, compiler-debt reduction under `haxe_go-vfp.8.7.13`, and selector isolation under `haxe_go-vfp.8.7.15.7` |
...
| `SDR-008` | retired `stdlib_symbols` library surfaces | Retire the group completely. Emit upstream `haxe.ds.Option` through the ordinary enum pipeline, remove the empty `Std` nominal carrier and false selectors, and keep every legitimate metadata/representation capability independently named in the intrinsic registry. Track `Std.parseInt` and `haxe.Log.trace` as explicit source-ownership migration calls rather than preserving a compatibility bucket. | Preserve the mixed blob, invent a special Option carrier, keep an empty compatibility group, stage an incomplete `Std`, or attach serialization to Type/Reflect | Sibling-target staged-source precedent; option snapshot + portable parity; bidirectional registry/provenance/debt contracts; `haxe_go-vfp.8.7.15.7`; Std/Log follow-up `haxe_go-vfp.8.7.22` |
