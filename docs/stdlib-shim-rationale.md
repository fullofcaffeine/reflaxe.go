...
| `atomic` | `haxe.atomic.*` operations | thin forwarding group | Semantic-diff + race | Migration required | The compiler functions only forward typed runtime operations and consume no compiler metadata. | `haxe_go-vfp.8.7.9` |
...
| `ds` | `haxe.ds.*Map`, `List`, enum maps | 149 | Snapshot + semantic-diff dependency | Migration required | Deterministic carriers may use narrow typed storage helpers; collection APIs and algorithms belong in staged Haxe. | `haxe_go-vfp.8.7.10`, `haxe_go-vfp.8.7.17` |
...
| `SDR-004` | `ds` (`haxe.ds.*Map`, `List`) | Move public carriers and algorithms to staged Haxe; use narrow typed native storage only where identity or representation requires it. | Preserve generated map/list classes, extern-only containers, staged source plus typed storage capabilities | `ds_maps_list_contract`; migrations `haxe_go-vfp.8.7.10` and `haxe_go-vfp.8.7.17` |
