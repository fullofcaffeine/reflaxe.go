...
| `io` | `haxe.io.Bytes`, buffers, input/output base wiring | 108 | Snapshot + semantic-diff dependency | Split: keep representation-sensitive core, migrate algorithmic helpers when parity is proven | Shared representation boundary still matters for bytes shape and inherited IO types, but now that framework-owned `__go__` is real again the algorithmic leaf helpers no longer need to stay in `GoCompiler` just to reach same-package generated types. First extraction landed for `Bytes.ofHex`, `Bytes.toHex`, and `BytesBuffer` leaf operations via `hxrt`; RawNative/cache-coupled string helpers remain compiler-owned for now. | `haxe.go-czm`, `haxe.go-14as.50`, `haxe.go-14as.51`, `haxe.go-14as.52`, `haxe.go-14as.54` |
...
| `http` | `sys.Http` request/callback/proxy contract | 542 | Semantic-diff | Keep core choreography; audit leaf helper extraction | Request sequencing, callback timing, and response/body normalization are still one semantic contract, but payload/proxy leaf helpers can now be reviewed separately because framework-owned `__go__` no longer forces everything through compiler-emitted raw blocks. | `haxe.go-14as.50`, `haxe.go-14as.53` |
...
| `SDR-003` | `io` (`haxe.io.Bytes*`, stream helpers, encoding edges) | Keep compiler ownership for representation-sensitive core and reopen algorithmic helpers for runtime/helper extraction. First extraction landed for `Bytes.ofHex`, `Bytes.toHex`, and `BytesBuffer` leaf helpers; RawNative/cache-coupled string paths remain compiler-owned. | Compiler builtin lowering, extern/runtime package, `std/_std`, framework-owned raw helper islands | Semantic-diff contracts + shim-vs-direct benchmark (`test:perf:stdlib-shims`) + post-`__go__` audit (`haxe.go-14as.50`) + first Bytes extraction (`haxe.go-14as.51`) |
...
| `SDR-005` | `http` (`sys.Http`) | Keep compiler ownership for request/callback choreography, but re-open payload/proxy leaf helpers now that framework-owned `__go__` exists | Extern-only wrappers, raw-native app code, `std/_std`, framework-owned raw helper islands | Semantic-diff contracts (`http_request_callbacks_contract`, proxy/custom request tests) + post-`__go__` audit (`haxe.go-14as.50`) |
...
| `haxe.io.Bytes` RawNative/cache-coupled string helpers (`ofString`, `getString`, UTF16/raw-native conversions) | `GoCompiler` `io` shim group | Keep compiler-owned for now | These paths still co-own `__hx_raw` caching and RawNative/UTF16 encoding-tag behavior. They are not blocked by raw injection anymore; they are blocked by representation coupling. | `haxe.go-14as.54` |
| `haxe.io.Input` / `haxe.io.Output` helper loops (`readAll`, `readLine`, `readUntil`, `writeBytes`, `writeInput`, `readFullBytes`) | `GoCompiler` `io` shim group | Framework-owned `@:goAllowRaw` helper layer in staged std/runtime | These are mostly generic control-flow helpers around generated IO types. They need same-package access and typed imports, but not broad compiler knowledge. | `haxe.go-14as.52` |
| `sys.Http` payload/proxy leaf helpers | `GoCompiler` `http` shim group | Keep core choreography in compiler; extract leaf helpers where typed imports + framework-owned `__go__` reduce raw compiler bulk | Request lifecycle, callback order, and response normalization are still one semantic contract. Extraction should start at leaf helpers, not by splitting the whole surface prematurely. | `haxe.go-14as.53` |
