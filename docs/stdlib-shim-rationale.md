...
| `SDR-003` | `io` (`haxe.io.Bytes*`, stream helpers, encoding edges) | Keep compiler ownership for representation-sensitive core and reopen algorithmic helpers for framework-owned `@:goAllowRaw` + `GoInjection.__go__` extraction | Compiler builtin lowering, extern/runtime package, `std/_std`, framework-owned raw helper islands | Semantic-diff contracts + shim-vs-direct benchmark (`test:perf:stdlib-shims`) + post-`__go__` audit (`haxe.go-14as.50`) |
...
| --- | --- | --- | --- | --- |
| `haxe.io.Bytes` algorithmic helpers (`ofString`, `ofHex`, `toHex`, `getString`, UTF16/raw-native conversions, `BytesBuffer` helpers) | `GoCompiler` `io` shim group | Framework-owned `@:goAllowRaw` helper layer plus existing typed imports/runtime helpers | These helpers manipulate generated `haxe__io__Bytes` / `haxe__io__BytesBuffer` shapes directly but do not inherently require compiler-context decisions. Plain externs are awkward because the shapes are generated, yet compiler ownership is no longer the only option. | `haxe.go-14as.51` |
