...
| `net_socket` | `sys.net.Host`, `sys.net.Socket`, `sys.net.UdpSocket`, TLS socket composition | 0 compiler declarations (group retired) | Semantic-diff + snapshot runtime + direct race/cross-build | Migrated (canonical staged std + typed runtime capabilities) | Staged source owns public objects, stream/error policy, address construction, select identity, TLS configuration, and accepted SSL identity. Footprint-explicit `socket.go`, build-tagged broadcast adapters, and `socket_ssl.go` own only DNS/OS transport, partial-I/O/deadline/readiness behavior, socket options, and TLS resources over typed handles. | `haxe_go-vfp.8.7.14`, audited by `haxe_go-vfp.10.4` |
