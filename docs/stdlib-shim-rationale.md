...
| `stdlib_symbols` | `Std`, `Date`, `Math`, `Reflect`, Unicode, Option, Type metadata | 455 | Semantic-diff | Migration required except exact registered metadata/representation primitives | Date, Math, Unicode algorithms, and runtime reflection remain source/runtime migration work. Crypto, XML, and zip have left this group. Only separately registered type metadata, type-test, string representation, exception carrier, and Rest primitives are admitted. | `haxe_go-vfp.8.7.15` |
