...
| `sys` | `Sys`, `sys.io.File`, `sys.io.Process` | 89 | Snapshot | Migrated (runtime-owned behavior) | Behavior now lives in `hxrt.Sys*`/`hxrt.File*`/`hxrt.Process*`; compiler shim generation retains type shape, Haxe error/EOF translation, and shared IO-helper adapters. The complete root surface now has typed symbols: cwd/time/path/stream behavior delegates to hxrt, `setTimeLocale` reports explicit unavailability, and `cpuTime` fails at Haxe compile time instead of substituting wall time. Direct `sys.io.FileInput` / `FileOutput` / `FileSeek` parity rides on this layer through runtime-backed helpers. | `haxe.go-7zy.11` (completed 2026-02-19), `haxe.go-14as.17`, `haxe_go-vfp.8.7.1`, `haxe_go-vfp.8.7.2` |
