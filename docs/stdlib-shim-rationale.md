...
| `SDR-010` | stack/main-loop `haxe.misc` surfaces (`haxe.CallStack`, `haxe.NativeStackTrace`, `haxe.EntryPoint`, `haxe.MainLoop`, `haxe.Timer`) | Split the tranche honestly: keep deterministic stack fallbacks in staged std under target-sensitive snapshot coverage, while direct event-loop surfaces stay explicit compile-only blockers until the runtime contract is real | Leave as generic compile-only debt, force semantic-diff on event-loop scheduling prematurely, grow compiler-owned stack/event-loop shims without an ownership decision | Snapshot contract `stdlib/haxe_stack_loop_target_sensitive`, staged std overrides `std/haxe/CallStack.cross.hx` + `std/haxe/NativeStackTrace.cross.hx`, direct blocker `haxe.go-dt4s`, cross-target precedent from `haxe.rust/std/haxe/CallStack.cross.hx` |
