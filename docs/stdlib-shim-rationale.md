...
| `SDR-010` | stack/main-loop `haxe.misc` surfaces (`haxe.CallStack`, `haxe.NativeStackTrace`, `haxe.EntryPoint`, `haxe.MainLoop`, `haxe.Timer`) | Split the tranche honestly: keep deterministic stack fallbacks in staged std under target-sensitive snapshot coverage, keep direct event-loop surfaces explicitly unsupported until `haxe.MainLoop` is integrated with the now-runtime-backed `sys.thread` contract | Leave as generic compile-only debt, force semantic-diff on event-loop scheduling prematurely, grow compiler-owned stack/event-loop shims without a runtime ownership decision | Snapshot contract `stdlib/haxe_stack_loop_target_sensitive`, staged std overrides `std/haxe/CallStack.cross.hx` + `std/haxe/NativeStackTrace.cross.hx`, explicit unsupported negative fixtures `negative/direct_haxe_entrypoint_unsupported`, `negative/direct_haxe_mainloop_unsupported`, `negative/direct_haxe_timer_unsupported`, follow-up `haxe.go-14as.69`, cross-target precedent from `haxe.rust/std/haxe/CallStack.cross.hx` |
