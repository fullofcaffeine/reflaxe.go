...
| retired `http` | `sys.Http` request/callback/proxy contract | 0 (compiler group retired) | Semantic-diff + snapshot + direct runtime/race | Migrated (canonical staged std + typed runtime capabilities) | `std/go/_std/sys/Http.hx` owns request selection, payload/header/multipart policy, callback order, Output lifecycle, response normalization, and status/error handling. Typed `std/hxrt/http` handles expose footprint-explicit Go URL/transport work and bounded upload chunks in `runtime/hxrt/http.go`; no generated public HTTP type or helper island remains. | `haxe_go-vfp.8.7.12`, audited by `haxe_go-vfp.10.4` |
...
| `net_socket` | `sys.net.Host`, `sys.net.Socket`, `sys.net.UdpSocket`, TLS socket composition | 0 compiler declarations (group retired) | Semantic-diff + snapshot runtime + direct race/cross-build | Migrated (canonical staged std + typed runtime capabilities) | Staged source owns public objects, stream/error policy, bind/listen sequencing, address construction, select identity, TLS configuration, and accepted SSL identity. Footprint-explicit `socket.go`, build-tagged broadcast/listener/readiness adapters, and `socket_ssl.go` own only DNS/OS transport, pre-listen descriptors, transactional connection installation, partial-I/O/deadline/readiness behavior, typed TCP/TLS shutdown and socket options, and TLS resources over typed handles. | `haxe_go-vfp.8.7.14`, audited by `haxe_go-vfp.10.4`; server lifecycle `haxe_go-vfp.10.9.5`; real readiness `haxe_go-vfp.10.9.6`; TLS controls `haxe_go-vfp.10.9.7`; convergence review `haxe_go-vfp.10.9.8` |
...
| `SDR-007` | retired `net_socket` group (`sys.net.Host`, `sys.net.Socket`, `sys.net.UdpSocket`, `sys.ssl.Socket` composition) | Keep the complete public APIs and Haxe-facing policy in canonical staged source. Cross only typed opaque socket/certificate/key/SNI handles and concrete result carriers into footprint-explicit `socket.go`, build-tagged OS adapters, `ssl.go`, and `socket_ssl.go`; compiler ownership is retired. | Keep compiler classes, retain raw injection or `Dynamic` native handles, extern-only wrappers, move public objects/exceptions into `hxrt`, use staged source plus typed handles, or fake readiness/control success from connection presence | Host/TCP/readiness semantic-diff, UDP/TLS/SNI and nonblocking runtime snapshots, selective-runtime positive/negative evidence, direct close/timeout/saturation/drain/OOB/TLS-control/race tests, repeated active/goroutine/Linux-descriptor convergence, sibling staged-source precedent, and the permanent compiler-debt reduction under `haxe_go-vfp.8.7.14` |
