...
| `sys` | `Sys`, `sys.io.Process` | 485 current span (138 `GoRaw` sites) | Snapshot | Keep thin adapters while source migrations remain open | Root/process behavior lives in `hxrt.Sys*` and `hxrt.Process*`; compiler generation retains only their type/call-shape and exception/EOF adapters. Root standard streams construct the staged File wrappers. The complete root surface has typed symbols, `setTimeLocale` reports explicit unavailability, and `cpuTime` fails at Haxe compile time instead of substituting wall time. | `haxe.go-7zy.11` (completed 2026-02-19), `haxe_go-vfp.8.7.1`, `haxe_go-vfp.8.7.2`, `haxe_go-vfp.8.7.5` |
...
| `SDR-002` | `sys` (`Sys`, `sys.io.Process`) | Keep thin compiler wrappers and move native behavior to `hxrt` (`Sys*`, `Process*`) while their source migrations remain open | Compiler shim, direct externs, canonical `std/go/_std` | Snapshot parity + migration log (`haxe.go-7zy.11`) and root/process follow-ups through `haxe_go-vfp.8.7.2` |
...
- `std/go/_std/sys/io` owns the File public API and stream semantics. `src/reflaxe/go/GoCompiler.hx` owns lowering and the still-open root `Sys` / `Process` type and status adapters; it is no longer a File API owner.
- `lowerSysStdlibShimDecls` must remain adapter-only for root `Sys` and `Process`: OS/process behavior belongs in `hxrt`, and File behavior belongs in staged std over typed `hxrt` capabilities.
...
- The same rule covers the completed root API: generated declarations only adapt Haxe carrier/error shape; `hxrt` owns clocks, paths, cwd mutation, standard descriptors, and byte input. `Sys.cpuTime` is the explicit exception: it is rejected at Haxe compile time because no Go-stdlib implementation can satisfy the CPU-time contract. See [Portable root `Sys` contract](portable-sys-contract.md).
