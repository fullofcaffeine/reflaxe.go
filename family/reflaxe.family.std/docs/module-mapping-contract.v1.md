...
| --- | --- | --- | --- | --- |
| `Math` | `mixed` (`haxe_source` + typed native bindings) | `std/go/_std/Math.hx` owns Haxe rounding, finiteness, NaN propagation, and signed-zero policy | Typed `std/hxrt/math` bindings call Go `math` / `math/rand`; only three Int-returning rounders require `runtime/hxrt/math.go` | `math_source_owned`, `numeric_edge_cases`, `stringtools_math`, `stdlib/date_math_source_owned` |
| `Std` | `mixed` (exact compiler primitives + runtime helpers + explicit migration calls) | Core typed lowering owns only individually registered type/representation operations; there is no generated `Std` carrier or broad stdlib-symbol declaration emitter | `runtime/hxrt/string.go`, `runtime/hxrt/exception.go`, core helpers; `Std.parseInt` remains explicit migration work | `exception_api_contract`, `std_is_of_type_contract`, `std_is_of_type_runtime_core_abstract_contract`, `typed_nil_dynamic_string_contract` |
| `haxe.ds.Option` | `haxe_source` | Upstream Haxe `haxe/ds/Option.hx` through ordinary enum lowering | No dedicated compiler carrier or runtime helper | `option_date_path`, `stdlib/option_enum_basic` |
...
| `Sys` | `mixed` (`compiler wrapper` + `runtime_binding`) | `src/reflaxe/go/GoCompiler.hx` (`lowerSysStdlibShimDecls`) | `runtime/hxrt/sys.go`; the complete root surface is specified in [Portable root `Sys` contract](portable-sys-contract.md). Portable `putEnv` discards the retained native error for Haxe 4.3.7 eval parity; `setTimeLocale` reports `false`; `cpuTime` fails at Haxe compile time; clocks, paths, cwd, blocking, and non-owning standard streams delegate to typed hxrt helpers. | `root_sys_portable_contract`, `sys_io_roundtrip`, `file_error_semantics_contract`, `sys_sleep_contract`, `sys/root_sys_portable`, `negative/sys_cpu_time_unsupported` |
...
| `sys.io.File` | `runtime_binding` | `src/reflaxe/go/GoCompiler.hx` (`lowerSysStdlibShimDecls` type/error adapters) | `runtime/hxrt/sys.go` (`FileSaveContent`, `FileGetContent`, `FileGetBytes`, `FileSaveBytes`, `FileCopy`, `OpenFile*Output`, `OpenFileInput`); OS failures remain exceptions | `file_read_write_contract`, `file_error_semantics_contract`, `semantic_diff/sys_db_io_contract`, `sys/file_error_semantics`, `stdlib/sys_db_io_direct` |
| `sys.io.Process` | `runtime_binding` | `src/reflaxe/go/GoCompiler.hx` (`lowerSysStdlibShimDecls` type/error/stream adapters) | `runtime/hxrt/process.go` (`NewProcess`, `Process`, `ProcessInput`, `ProcessOutput`); startup/pipe/read/wait failures, EOF, and exit status remain distinct | `process_echo_contract`, `process_error_semantics_contract`, `sys/process_error_semantics` |
...
| `sys.net.Socket` | `mixed` (`haxe_source` + `runtime_binding`) | `std/go/_std/sys/net/Socket.hx` and `std/sys/net/_SocketIO.hx` own the public API, stream adapters, Haxe exceptions, and select object identity | Typed opaque handles and concrete result carriers in `std/hxrt/net` over footprint-explicit `runtime/hxrt/socket.go`; no `net_socket` compiler emitter or authority remains | `socket_advanced_contract`, `socket_loopback_contract`, `sys/socket_input_service_surface`, `core/runtime_hxrt_infer_socket`, direct runtime race tests; source migration `haxe_go-vfp.8.7.14` |
...
| `sys.ssl` direct surfaces (`Certificate`, `Digest`, `DigestAlgorithm`, `Key`, `Socket`) | `mixed` (`haxe_source` + `runtime_binding`) | `std/go/_std/sys/ssl/*.hx` owns the public API and composes `sys.ssl.Socket` over the shared source-owned `sys.net.Socket`; `DigestAlgorithm` is fully source-owned | Typed carriers in `std/hxrt/ssl`; footprint-explicit `runtime/hxrt/ssl.go` owns certificate/key/digest primitives while `runtime/hxrt/socket_ssl.go` owns TLS handshake, peer-certificate, and SNI composition over the shared socket handle | `stdlib/sys_ssl_leaf_direct`, `stdlib/sys_ssl_socket_direct`, `stdlib/sys_ssl_socket_sni_direct`, `core/runtime_hxrt_infer_socket_ssl`, direct runtime race tests, `semantic_diff/sys_net_address_ssl_digest_algorithm_contract`, `stdlib/sys_net_address_ssl_digest_algorithm_direct`; policy spike: `docs/spikes/ssl-udp-semantic-diff-spike.md`; socket source migration `haxe_go-vfp.8.7.14` |
