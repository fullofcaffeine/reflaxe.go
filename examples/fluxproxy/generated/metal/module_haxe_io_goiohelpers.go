...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_90 := false
		var hx_try_value_91 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
			...
			switch hx_typed_93 := hx_caught_92.(type) {
			case *haxe__io__Eof:
				e := hx_typed_93
				...
				hx_try_value_91 = partial
				hx_try_return_90 = true
				return
			default:
				hxrt.Throw(hx_caught_92)
			}
		})
		if hx_try_return_90 {
			return hx_try_value_91
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteInput(self haxe__io__Output, i haxe__io__Input, bufsize int) {
	...
	for true {
		...
		}, func(hx_caught_94 any) {
			switch hx_typed_95 := hx_caught_94.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_95
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_94)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_96 *haxe__io__Bytes
	if encoding == nil {
		hx_if_96 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_96 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_96
