...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_118 := false
		var hx_try_value_119 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
		...
		}, func(hx_caught_120 any) {
			switch hx_typed_121 := hx_caught_120.(type) {
			case *haxe__io__Eof:
				e := hx_typed_121
				...
				hx_try_value_119 = partial
				hx_try_return_118 = true
				return
			default:
				hxrt.Throw(hx_caught_120)
			}
		})
		if hx_try_return_118 {
			return hx_try_value_119
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_124 *haxe__io__Bytes
	if encoding == nil {
		hx_if_124 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_124 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_124
