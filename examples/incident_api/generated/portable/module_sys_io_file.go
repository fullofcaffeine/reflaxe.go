...
func sys__io__File_saveBytes(path *string, bytes *haxe__io__Bytes) {
	...
	hxrt.FileWriteByteValues(path, func(hx_lambda_raw_144 []any) []int {
		hx_lambda_out_145 := make([]int, 0, len(hx_lambda_raw_144))
		for _, hx_lambda_item_146 := range hx_lambda_raw_144 {
			hx_lambda_out_145 = append(hx_lambda_out_145, func(hx_value_147 any) int {
				if hx_value_147 == nil {
					var hx_zero_148 int
					return hx_zero_148
				}
				return hx_value_147.(int)
			}(hx_lambda_item_146))
		}
		return hx_lambda_out_145
	}(values.Values()))
}
