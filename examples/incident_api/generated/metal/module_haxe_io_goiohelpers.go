...
func haxe__io__GoIoHelpers_inputReadAll(self haxe__io__Input, bufsize int) *haxe__io__Bytes {
	...
	for true {
		...
		}, func(hx_caught_154 any) {
			switch hx_typed_155 := hx_caught_154.(type) {
			case *haxe__io__Eof:
				hx_tmp := hx_typed_155
				_ = hx_tmp
				done = true
			default:
				hxrt.Throw(hx_caught_154)
			}
		})
		if done {
			break
...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_156 := false
		var hx_try_value_157 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
			...
			case *haxe__io__Eof:
				...
				hx_try_value_157 = partial
				hx_try_return_156 = true
				return
			default:
				hxrt.Throw(hx_caught_158)
			}
		})
		if hx_try_return_156 {
			return hx_try_value_157
		}
		if ended {
			break
		}
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_162 *haxe__io__Bytes
	if encoding == nil {
		hx_if_162 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_162 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_162
