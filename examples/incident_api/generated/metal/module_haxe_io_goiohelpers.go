...
func haxe__io__GoIoHelpers_inputReadLine(self haxe__io__Input) *string {
	...
	for true {
		...
		_ = last
		ended := false
		hx_try_return_59 := false
		var hx_try_value_60 *string
		hxrt.TryCatch(func() {
			last = self.readByte()
			if last == 10 {
				ended = true
			...
			case *haxe__io__Eof:
				...
				hx_try_value_60 = partial
				hx_try_return_59 = true
				return
			default:
...
func haxe__io__GoIoHelpers_outputWriteString(self haxe__io__Output, s *string, encoding *haxe__io__Encoding) {
	...
	var hx_if_65 *haxe__io__Bytes
	if encoding == nil {
		hx_if_65 = haxe__io__Bytes_ofString(s)
	} else {
		hx_if_65 = haxe__io__Bytes_ofString(s, encoding)
	}
	bytes := hx_if_65
