/**
	What:
	- Owns the Go-target implementation of `DateTools` formatting and date helpers.

	Why:
	- The mainstream Haxe stdlib implementation cannot be used unchanged on `haxe.go`. because its formatting and parsing code must stay within the Date, Std, and StringTools operations the Go backend lowers reliably.

	How:
	- Preserve the upstream public API while expressing each helper with already-supported portable operations.
**/
class DateTools {
