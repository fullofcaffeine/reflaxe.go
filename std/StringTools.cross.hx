...
/**
	Go target override for `StringTools`.

	Why
	- The portable target should not keep basic library helpers in compiler-emitted shims.
	- The previous compiler-owned subset only covered `trim`, `startsWith`, and `replace`,
	  which left direct `contains`/`endsWith` calls falling back to unsupported lowering.

	How
	- Keep macro evaluation safe with pure Haxe fallbacks.
	- Use typed Go `strings` package bindings at target runtime for the supported helper subset.
**/
class StringTools {
