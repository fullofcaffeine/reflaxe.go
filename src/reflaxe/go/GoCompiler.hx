...
class GoCompiler {
	...
	function lowerBlock(exprs:Array<TypedExpr>):Array<GoStmt> {
		...
		popLocalScope();
		return pruneRedundantBlankIdentifierGuards(out);
	}

	function pruneRedundantBlankIdentifierGuards(stmts:Array<GoStmt>):Array<GoStmt> {
		var result = new Array<GoStmt>();
		var index = 0;
		while (index < stmts.length) {
			var current = stmts[index];
			if (index + 1 < stmts.length) {
				switch ([current, stmts[index + 1]]) {
					case [
						GoStmt.GoVarDecl(varName, _, _, _),
						GoStmt.GoAssign(GoExpr.GoIdent("_"), GoExpr.GoIdent(guardName))
					] if (varName == guardName && isIdentReadLaterInStmts(varName, stmts, index + 2)):
						result.push(current);
						index += 2;
						continue;
					case _:
	...
	function isIdentReadLaterInStmts(name:String, stmts:Array<GoStmt>, startIndex:Int):Bool {
		for (index in startIndex...stmts.length) {
			if (stmtReadsIdent(stmts[index], name)) {
				return true;
			}
		}
		return false;
	...
	function stmtReadsIdent(stmt:GoStmt, name:String):Bool {
		return switch (stmt) {
			case GoStmt.GoVarDecl(_, _, value, _): value != null && exprReadsIdent(value, name);
			case GoStmt.GoAssign(left, right): lvalueReadsIdent(left, name) || exprReadsIdent(right, name);
			case GoStmt.GoExprStmt(expr):
				exprReadsIdent(expr, name);
			case GoStmt.GoGoStmt(call):
				exprReadsIdent(call, name);
			case GoStmt.GoDeferStmt(call):
				exprReadsIdent(call, name);
			case GoStmt.GoSendStmt(channel, value): exprReadsIdent(channel, name) || exprReadsIdent(value, name);
			case GoStmt.GoRaw(_):
				false;
			case GoStmt.GoWhile(cond, body): exprReadsIdent(cond, name) || blockReadsIdent(body, name);
			case GoStmt.GoRangeStmt(keyName, valueName, source, _, body):
				if (keyName == name || valueName == name) {
					exprReadsIdent(source, name);
				} else {
					exprReadsIdent(source, name) || blockReadsIdent(body, name)
					;
				}
			case GoStmt.GoIf(cond, thenBody, elseBody): exprReadsIdent(cond,
					name) || blockReadsIdent(thenBody, name) || (elseBody != null && blockReadsIdent(elseBody, name));
			case GoStmt.GoSwitch(value, cases, defaultBody):
				if (exprReadsIdent(value, name)) {
					true;
				} else {
					var matched = false;
					for (caseEntry in cases) {
						for (caseValue in caseEntry.values) {
							if (exprReadsIdent(caseValue, name)) {
								matched = true;
								break;
							}
						}
						if (matched || blockReadsIdent(caseEntry.body, name)) {
							matched = true;
							break;
						}
					}
					matched || (defaultBody != null && blockReadsIdent(defaultBody, name))
					;
				}
			case GoStmt.GoTypeSwitch(value, bindingName, cases, defaultBody):
				if (exprReadsIdent(value, name)) {
					...
					for (caseEntry in cases) {
						...
						if (blockReadsIdent(caseEntry.body, name)) {
							matched = true;
							break;
						}
					}
					matched || (defaultBody != null && bindingName != name && blockReadsIdent(defaultBody, name))
					;
				}
			case GoStmt.GoSelect(cases):
				var matched = false;
				for (caseEntry in cases) {
					var clauseReads = selectClauseReadsIdent(caseEntry.clause, name);
					var bodyReads = selectCaseBodyReadsIdent(caseEntry, name);
					if (clauseReads || bodyReads) {
						matched = true;
						break;
					}
				}
			...
			case GoStmt.GoContinue:
				false;
			case GoStmt.GoReturn(expr): expr != null && exprReadsIdent(expr, name);
		}
	}

	function blockReadsIdent(stmts:Array<GoStmt>, name:String):Bool {
		for (stmt in stmts) {
			if (stmtReadsIdent(stmt, name)) {
				return true;
			}
		}
		return false;
	...
	function blockDeclaresIdent(stmts:Array<GoStmt>, name:String):Bool {
		for (stmt in stmts) {
			if (stmtDeclaresIdent(stmt, name)) {
				return true;
			}
		}
		return false;
	...
	function stmtDeclaresIdent(stmt:GoStmt, name:String):Bool {
		return switch (stmt) {
			case GoStmt.GoVarDecl(varName, _, _, _):
				varName == name;
			case GoStmt.GoWhile(_, body):
				blockDeclaresIdent(body, name);
			case GoStmt.GoRangeStmt(keyName, valueName, _, _, body): keyName == name || valueName == name || blockDeclaresIdent(body, name);
			case GoStmt.GoIf(_, thenBody, elseBody): blockDeclaresIdent(thenBody, name) || (elseBody != null && blockDeclaresIdent(elseBody, name));
			case GoStmt.GoSwitch(_, cases, defaultBody): var declared = false; for (caseEntry in cases) {
					if (blockDeclaresIdent(caseEntry.body, name)) {
						declared = true;
						break;
					}
				} declared || (defaultBody != null && blockDeclaresIdent(defaultBody, name));
			case GoStmt.GoTypeSwitch(_, bindingName, cases, defaultBody):
				if (bindingName == name) {
					true;
				} else {
					var declared = false;
			...
			case GoStmt.GoSelect(cases):
				...
				for (caseEntry in cases) {
					switch (caseEntry.clause) {
						case GoSelectClause.GoSelectRecvAssign(GoExpr.GoIdent(targetName), _, true) if (targetName == name):
							declared = true;
						case _:
					}
					if (declared || blockDeclaresIdent(caseEntry.body, name)) {
						declared = true;
						break;
					}
				}
	...
	function functionLiteralReadsIdent(params:Array<GoParam>, body:Array<GoStmt>, name:String):Bool {
		for (param in params) {
			if (param.name == name) {
				return false;
			}
		}
		if (blockDeclaresIdent(body, name)) {
			return false;
		}
		return blockReadsIdent(body, name);
	}

	function selectClauseReadsIdent(clause:GoSelectClause, name:String):Bool {
		return switch (clause) {
			case GoSelectClause.GoSelectSend(channel, value): exprReadsIdent(channel, name) || exprReadsIdent(value, name);
			case GoSelectClause.GoSelectRecv(recv):
				exprReadsIdent(recv, name);
			case GoSelectClause.GoSelectRecvAssign(target, recv, _): lvalueReadsIdent(target, name) || exprReadsIdent(recv, name);
			case GoSelectClause.GoSelectDefault:
				false;
		};
	}

	function selectCaseBodyReadsIdent(caseEntry:GoSelectCase, name:String):Bool {
		return switch (caseEntry.clause) {
			case GoSelectClause.GoSelectRecvAssign(GoExpr.GoIdent(targetName), _, true) if (targetName == name):
				false;
			case _:
				blockReadsIdent(caseEntry.body, name);
		};
	}

	function lvalueReadsIdent(expr:GoExpr, name:String):Bool {
		return switch (expr) {
	...
	function exprReadsIdent(expr:GoExpr, name:String):Bool {
		return switch (expr) {
			case GoExpr.GoIdent(ident):
				ident == name;
			case GoExpr.GoIntLiteral(_):
				false;
			case GoExpr.GoFloatLiteral(_):
				false;
			case GoExpr.GoBoolLiteral(_):
				false;
			case GoExpr.GoStringLiteral(_):
				false;
			case GoExpr.GoNil:
				false;
			case GoExpr.GoSelector(target, _):
				exprReadsIdent(target, name);
			case GoExpr.GoIndex(target, index): exprReadsIdent(target, name) || exprReadsIdent(index, name);
			case GoExpr.GoSlice(target, start, end): exprReadsIdent(target,
					name) || (start != null && exprReadsIdent(start, name)) || (end != null && exprReadsIdent(end, name));
			case GoExpr.GoArrayLiteral(_, elements):
				var matched = false;
				for (element in elements) {
					if (exprReadsIdent(element, name)) {
						matched = true;
						break;
					}
				}
			...
			case GoExpr.GoFuncLiteral(params, _, body):
				functionLiteralReadsIdent(params, body, name);
			case GoExpr.GoRaw(_):
				false;
			case GoExpr.GoTypeAssert(value, _):
				exprReadsIdent(value, name);
			case GoExpr.GoRecvExpr(channel):
				exprReadsIdent(channel, name);
			case GoExpr.GoUnary(_, value):
				exprReadsIdent(value, name);
			case GoExpr.GoBinary(_, left, right): exprReadsIdent(left, name) || exprReadsIdent(right, name);
			case GoExpr.GoCall(callee, args):
				if (exprReadsIdent(callee, name)) {
					true;
				} else {
					var matched = false;
					for (arg in args) {
						if (exprReadsIdent(arg, name)) {
							matched = true;
							break;
						}
					}
