...
class GoBuildContextResolver {
	...
	public static inline final SURFACE_CONTRACT_REPORT_DEFINE:GoCompilerDefine = GoCompilerDefine.DefineSurfaceContractReport;
	...
	public static function resolve():GoBuildContext {
		...
		return new GoBuildContext(profile, policyResolution, normalizeGoModuleName(Context.definedValue(GO_MODULE_DEFINE)), RawNativeModeResolver.resolve(),
			...
			Context.defined(SURFACE_CONTRACT_REPORT_DEFINE), autoLoweringMode, optimizationPreset, portableStringFastpathEnabled,
			portableConcurrencyFastpathEnabled, Context.defined(NATIVE_STACK_TRACE_DEFINE), []);
	}
