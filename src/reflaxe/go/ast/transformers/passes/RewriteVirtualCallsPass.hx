...
class RewriteVirtualCallsPass implements IGoASTPass {
	...
	function stmtDeclaresIdent(stmt:GoStmt, ident:String):Bool {
		return switch (stmt) {
			...
			case GoStmt.GoAssign(left, right): exprDeclaresIdent(left, ident) || exprDeclaresIdent(right, ident);
			case GoStmt.GoExprStmt(expr):
			...
			case GoStmt.GoTypeSwitch(_, _, cases, defaultBody): var found = false; for (entry in cases) {
					if (stmtListDeclaresIdent(entry.body, ident)) {
						found = true;
						break;
					}
	...
	function exprDeclaresIdent(expr:GoExpr, ident:String):Bool {
		return switch (expr) {
			case GoExpr.GoFuncLiteral(params, _, body): var shadows = false; for (param in params) {
					if (param.name == ident) {
						shadows = true;
						break;
					}
			...
			case GoExpr.GoIndex(target, index): exprDeclaresIdent(target, ident) || exprDeclaresIdent(index, ident);
			case GoExpr.GoSlice(target, start, end): exprDeclaresIdent(target,
					ident) || (start != null && exprDeclaresIdent(start, ident)) || (end != null && exprDeclaresIdent(end, ident));
	...
	function stmtUsesIdent(stmt:GoStmt, ident:String):Bool {
		return switch (stmt) {
			case GoStmt.GoVarDecl(_, _, value, _): value != null && exprUsesIdent(value, ident);
			case GoStmt.GoAssign(left, right): exprUsesIdent(left, ident) || exprUsesIdent(right, ident);
			case GoStmt.GoExprStmt(expr):
			...
			case GoStmt.GoTypeSwitch(value, _, cases, defaultBody): var found = exprUsesIdent(value, ident); if (!found) {
					for (entry in cases) {
	...
	function exprUsesIdent(expr:GoExpr, ident:String):Bool {
		return switch (expr) {
			...
			case GoExpr.GoIndex(target, index): exprUsesIdent(target, ident) || exprUsesIdent(index, ident);
			case GoExpr.GoSlice(target, start, end): exprUsesIdent(target,
					ident) || (start != null && exprUsesIdent(start, ident)) || (end != null && exprUsesIdent(end, ident));
			case GoExpr.GoArrayLiteral(_, elements):
				var found = false;
				for (element in elements) {
