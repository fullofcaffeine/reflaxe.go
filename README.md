...
| --- | --- | --- | --- |
| `compile-only` | Module/API compiles and generated Go passes probe-level `go test`, but no runtime parity claim by itself | `python3 test/run-upstream-stdlib-sweep.py --modules-file test/upstream_std_modules_full.txt --strict --go-test` | `haxe.Int64`, `haxe.Int64Helper`, `haxe.atomic.AtomicObject`, `sys.ssl.Socket` |
